...
public class YupdCommand implements Callable<Integer> {
    ...
    @CommandLine.Option(names = {"--set"}, required = true, defaultValue = "${YUPD_SET}", split = "\\|\\|", splitSynopsisLabel = "||", description = "Allows setting YAML path expressions (e.g., metadata.name=new_name) or regular expressions (env: YUPD_SET)")
